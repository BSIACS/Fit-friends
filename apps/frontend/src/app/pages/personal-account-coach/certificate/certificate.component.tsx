...
import { deleteTrainersCertificateDataThunk, updateTrainerDataThunk, updateTrainersCertificateDataThunk } from '../../../store/slices/application.thunk';
...
export function CertificateComponent({ trainerId, certificateFileName }: CertificateComponentProps): JSX.Element {
  ...
  const deleteFileButtonClickHandler = (evt: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    dispatch(deleteTrainersCertificateDataThunk({ id: trainerId, deletedCertificateName: certificateFileName }))
  }
  ...
  const onSaveButtonHandler = (evt: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    ...
    if (loadFileInputRef.current?.files) {
      console.log(loadFileInputRef.current?.files[0]);
      const formData = new FormData();
      formData.set('id', trainerId);
      formData.set('updatedCertificateName', certificateFileName);
      formData.set('certificate', loadFileInputRef.current?.files[0]);

      dispatch(updateTrainersCertificateDataThunk({ formData: formData }));
    }
  }
    ...
    <li className="personal-account-coach__item">
      <div className="certificate-card" style={{ margin: '0 20px 0 0' }}>
        <div className="certificate-card__image">
          <Document file={`http://localhost:3042/assets/users-data/${trainerId}/certificates/${certificateFileName}`}>
