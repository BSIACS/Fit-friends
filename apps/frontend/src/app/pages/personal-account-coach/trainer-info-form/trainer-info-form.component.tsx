...
import styles from './trainer-info-form.component.module.css'
import { useAppDispatch } from '../../../hooks/useAppDispatch';
import { changeEditTrainerFormData } from '../../../store/slices/application.slice';
...
type TrainerFormInfoComponentProps = {
  user: UserData
}

export function TrainerFormInfoComponent({ user }: TrainerFormInfoComponentProps): JSX.Element {
  ...
  const userInfoEditSubmitHandler = (evt: React.MouseEvent<HTMLFormElement, MouseEvent>) => {
    ...
    const descriptionCurrentLength = descriptionRef.current?.value?.length as number;
    if (descriptionCurrentLength < 10 || descriptionCurrentLength > 140) {
    ...
    if (nameError.isError || descriptionError.isError || errorFlag) {
      return;
    }
  ...
  const descriptionInputChangeHandler = (evt: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    dispatch(changeEditTrainerFormData({ ...user, description: evt.currentTarget.value }));
  }
  ...
  const nameInputChangeHandler = (evt: React.ChangeEvent<HTMLInputElement>) => {
    ...
    dispatch(changeEditTrainerFormData({ ...user, name: evt.currentTarget.value }));
  }
  ...
  const trainingTypeInputChangeHandler = (evt: React.ChangeEvent<HTMLInputElement>) => {
    ...
    dispatch(changeEditTrainerFormData({ ...user, trainingType: [] }));

    if (trainingType.includes(value)) {
      const index = trainingType.indexOf(value);
      trainingType.splice(index, 1);
      dispatch(changeEditTrainerFormData({ ...user, trainingType: trainingType }));
    }
    else {
      dispatch(changeEditTrainerFormData({ ...user, trainingType: [...trainingType, value] }));
    }
  }

  const isReadyForTrainingChangeHandler = (evt: React.ChangeEvent<HTMLInputElement>) => {
    dispatch(changeEditTrainerFormData({ ...user, isReadyForTraining: !user.isReadyForTraining }));
  }
  ...
  const locationListItemClickHandler = (evt: React.MouseEvent<HTMLLIElement, MouseEvent>) => {
    dispatch(changeEditTrainerFormData({ ...user, location: evt.currentTarget.dataset.value }));
    setIsSelectLocationListVissible(false);
  }

  const sexListItemClickHandler = (evt: React.MouseEvent<HTMLLIElement, MouseEvent>) => {
    dispatch(changeEditTrainerFormData({ ...user, sex: evt.currentTarget.dataset.value }));
    setIsSelectSexListVissible(false);
  }

  const trainingLevelListItemClickHandler = (evt: React.MouseEvent<HTMLLIElement, MouseEvent>) => {
    dispatch(changeEditTrainerFormData({ ...user, trainingLevel: evt.currentTarget.dataset.value }));
  ...
  return (
    <section className="user-info">
      ...
      <form className="user-info__form" action="#" method="post" onSubmit={userInfoEditSubmitHandler}>
        ...
        <div className="user-info__section" style={{ marginBottom: '0px' }}>
          ...
          <div className={isEditable ? "custom-input user-info-edit__input" : 'custom-input custom-input--readonly user-info__input'} style={{ marginBottom: '10px' }}>
            <label>
              ...
              <span className="custom-input__wrapper">
                <input type="text" name="name" value={user.name ? user.name : ''} onChange={nameInputChangeHandler} disabled={!isEditable} />
              </span>
            </label>
          </div>
          <div>
          ...
          <div className="custom-textarea custom-textarea--readonly user-info__textarea" style={{ marginTop: '10px' }}>
            <label>
              <span className="custom-textarea__label">Описание</span>
              <textarea ref={descriptionRef} name="description" value={user.description} onChange={descriptionInputChangeHandler} disabled={!isEditable} />
            </label>
          </div>
        </div>
        <div>
        ...
        <div className="user-info-edit__section user-info-edit__section--status" style={{ marginTop: '10px' }}>
          ...
          <div className="custom-toggle custom-toggle--switch user-info-edit__toggle">
            <label>
              <input type="checkbox" name="isReadyForTraining" checked={user.isReadyForTraining ? user.isReadyForTraining : false} onChange={isReadyForTrainingChangeHandler}
                disabled={!isEditable} />
